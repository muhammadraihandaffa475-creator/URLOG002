...
@media (max-width: 768px) {
    ...
    .glass-card { padding: 15px; height: 100vh; width: 100vw; border-radius: 0; display: flex; flex-direction: column; margin: 0; border: none; }
    .admin-header { flex-shrink: 0; flex-direction: column; text-align: center; gap: 8px; padding-bottom: 10px; }
    .content-grid { grid-template-columns: 1fr; gap: 15px; display: flex; flex-direction: column; flex: 1; overflow-y: auto; overflow-x: hidden; padding-bottom: 20px; }
    .form-section { overflow: visible; flex-shrink: 0; padding-right: 0; }
    .table-container { flex: 1; min-height: 400px; overflow-y: auto; }
}
    ...
    <script>
        ...
        function getExpiringContracts(rows) {
            ...
            for(let i = 0; i < Math.min(rows.length, 5); i++) {
                ...
                const idx = rowUpper.findIndex(h => (h.includes('AKHIR') || h.includes('SELESAI') || h.includes('JATUH TEMPO') || h.includes('BATAS') || h.includes('BERAKHIR')) && !h.includes('STATUS') && !h.includes('KET'));
                const idxBastp = rowUpper.findIndex(h => h.includes('BASTP') || h.includes('SERAH TERIMA') || h.includes('PHO') || h.includes('BERITA ACARA') || h.includes('TGL BAST') || h.includes('TANGGAL BAST') || h.includes('STATUS') || h.includes('KETERANGAN') || h.includes('KET.'));
                if(idx !== -1) {
                    headerRowIdx = i;
                    dateColIdx = idx;
                    bastpColIdx = idxBastp;
                    break;
                }
            }
            ...
            for (let i = headerRowIdx + 1; i < rows.length; i++) {
                ...
                // Cek Status BASTP
                let isBastpClear = false;
                if (bastpColIdx !== -1 && row[bastpColIdx]) {
                    const bastpStr = row[bastpColIdx].trim();
                    const upperStr = bastpStr.toUpperCase();
                    if ((bastpStr.length > 4 && parseIndonesianDate(bastpStr)) || upperStr.includes('SUDAH') || upperStr.includes('SELESAI') || upperStr.includes('LENGKAP') || upperStr.includes('OK') || upperStr.includes('ADA')) isBastpClear = true;
                    if (upperStr.includes('BELUM')) isBastpClear = false;
                }
                if (isBastpClear) continue;

                const dateStr = row[dateColIdx].trim();
                let contractDate = parseIndonesianDate(dateStr);
                if (contractDate) {
                    contractDate.setHours(0, 0, 0, 0);
                    // LOGIKA: Tampilkan jika (Expired/Masa Lalu) ATAU (H-5 ke depan)
                    if (contractDate <= maxDate) {
                        ...
                        if (contractDate < today) {
                            ...
                            statusMsg = `Telat ${diffDays} hari (BASTP Belum)`;
                            badgeColor = "color: #b91c1c; background: #fecaca;"; // Merah gelap
                        } else {
                            // Akan jatuh tempo
                            const diffTime = Math.abs(contractDate - today);
                            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
                            statusMsg = diffDays === 0 ? "HARI INI (BASTP Belum)" : `${diffDays} hari lagi (BASTP Belum)`;
                            badgeColor = "color: #ef4444; background: #fef2f2;"; // Merah terang
                        }

                        expiringContracts.push({ name: contractName, date: dateStr, status: statusMsg, badgeColor: badgeColor });
                    }
                }
            }
        ...
        function parseIndonesianDate(dateStr) {
            ...
            let cleanStr = dateStr.toLowerCase().replace(/,/g, '');
            
            // 2. Format: DD/MM/YYYY or DD-MM-YYYY or DD.MM.YYYY
            let parts = cleanStr.match(/(\d{1,2})[\/\-\.](\d{1,2})[\/\-\.](\d{4})/);
            if (parts) return new Date(parts[3], parts[2] - 1, parts[1]);

            // 3. Format: DD Bulan YYYY
            let textParts = cleanStr.split(/[\s\-]+/);
            if (textParts.length >= 3) {
                const day = parseInt(textParts[0]);
                const monthStr = textParts[1];
                const year = parseInt(textParts[2]);
                const months = { 'januari': 0, 'februari': 1, 'maret': 2, 'april': 3, 'mei': 4, 'juni': 5, 'juli': 6, 'agustus': 7, 'september': 8, 'oktober': 9, 'november': 10, 'desember': 11, 'jan': 0, 'feb': 1, 'mar': 2, 'apr': 3, 'mei': 4, 'jun': 5, 'jul': 6, 'agu': 7, 'aug': 7, 'sep': 8, 'okt': 9, 'oct': 9, 'nov': 10, 'des': 11, 'dec': 11 };
        ...
        function showContractNotification(contracts) {
            ...
            contracts.forEach(c => {
                const badgeStyle = c.badgeColor 
                    ? `font-size: 0.7rem; font-weight: 700; padding: 2px 8px; border-radius: 20px; ${c.badgeColor}`
                    : "font-size: 0.7rem; font-weight: 700; color: #ef4444; background: #fef2f2; padding: 2px 8px; border-radius: 20px;";
                listHtml += `
                <li onclick="openPBJSpreadsheet()" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px solid #f1f5f9; font-size: 0.85rem; color: #475569; transition: background 0.2s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='transparent'" title="Klik untuk membuka spreadsheet">
            ...
            alertBox.innerHTML = `
                ...
                <div style="flex: 1;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                        <h4 style="margin: 0; font-size: 1rem; font-weight: 700; color: #0f172a;">Peringatan BASTP</h4>
                        <span style="font-size: 0.75rem; color: #94a3b8;">Baru saja</span>
                    </div>
                    <p style="margin: 0; font-size: 0.9rem; color: #64748b; line-height: 1.4;">
                        Terdapat <strong style="color: #ef4444;">${contracts.length} kontrak</strong> dengan BASTP belum clear (Hampir/Sudah Jatuh Tempo).
                    </p>
                    ${listHtml}
                </div>
        ...
        function openPBJSpreadsheet() {
            const url = localStorage.getItem('PBJ_Url');
            if(url) {
                window.open(url, '_blank');
            } else {
                alert("Link Spreadsheet PBJ belum disetting.");
            }
        }
        ...
        async function testNotification() {
            ...
            const dummyContracts = [
                { name: "Contoh: Laptop (Akan Jatuh Tempo)", status: "HARI INI (BASTP Belum)", badgeColor: "color: #ef4444; background: #fef2f2;" }, 
                { name: "Contoh: Gedung (Sudah Expired)", status: "Telat 5 hari (BASTP Belum)", badgeColor: "color: #b91c1c; background: #fecaca;" }
            ];
        ...
        function sendBrowserNotification(contracts) {
            ...
            const title = "Admin: Peringatan BASTP";
            const options = {
                body: `Perhatian! Ada ${contracts.length} kontrak BASTP belum clear (Hampir/Sudah Jatuh Tempo).`,
